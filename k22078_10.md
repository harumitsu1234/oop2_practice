...
・watchdog機能の実装
指定されたディレクトリに画像を追加すると動作させたいプログラムが動くものを実装。

・watchdogとメンバーの作ってくれた画像処理のコードをマージ
メンバーに、画像を入力し処理をしたあと画像を返すプログラムをお願いした。
watchdogのメインコードを画像のパスを取得できるよう修正し、そのパスをメンバーの処理関数に渡すようにした。
画像処理のコードでも統一した形で画像の名前を返せるように修正を行なった。
